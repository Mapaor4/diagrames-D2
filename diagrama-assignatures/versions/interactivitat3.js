class InteractiveSVG {
    ...
    // 1. Get direct connections and related nodes
    ...
    // 2. If the hovered node is a container, add its descendants and their connections
    if (esContenidor) {
        ...
        descendents.forEach(classeFill => {
            ...
            if (elementFill) {
                ...
                // Get direct connections of the descendant
    ...
    // 3. NEW: Check each related node (neighbor) to see if it's a container
    nodesRelacionats.forEach(relatedNode => {
        if (relatedNode.classList.contains('diagram-container')) {
            const relatedClasseOriginal = Array.from(relatedNode.classList).find(c => this.esBase64Valid(c));
            if (!relatedClasseOriginal) return;

            const relatedNodeDecodificat = this.decodificarBase64(relatedClasseOriginal);

            // Add all descendants of the neighbor container
            const descendents = this.obtenirDescendents(relatedClasseOriginal);
            descendents.forEach(classeFill => {
                const elementFill = document.querySelector(`.${CSS.escape(classeFill)}`);
                if (elementFill) {
                    elementsAMostrar.add(elementFill);
            ...
            // Add internal connections within the neighbor container's hierarchy
            this.connexionsMap.forEach((info, classeConnexio) => {
                if (!info) return;
                const { startNode, endNode } = info;

                // Check if both nodes are within the container's hierarchy
                if (startNode.startsWith(`${relatedNodeDecodificat}.`) && 
                    endNode.startsWith(`${relatedNodeDecodificat}.`)) {
                    const elementConnexio = document.querySelector(`.${CSS.escape(classeConnexio)}`);
                    if (elementConnexio) {
                        elementsAMostrar.add(elementConnexio);
                    }
                }
            });
        }
