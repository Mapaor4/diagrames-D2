class InteractiveSVG {
    ...
    resaltar(node) {
        ...
        // 3. Processar nodes relacionats per afegir descendents i connexions internes
        nodesRelacionats.forEach(nodeRelacionat => {
            const classeRelacionat = Array.from(nodeRelacionat.classList).find(c => this.esBase64Valid(c));
            if (!classeRelacionat || !this.containerMap.has(classeRelacionat)) return;
    
            const nodeRelacionatDecodificat = this.decodificarBase64(classeRelacionat);
    
            // Afegir tots els descendents del node relacionat (contenidor)
            const descendentsRelacionat = this.obtenirDescendents(classeRelacionat);
            descendentsRelacionat.forEach(classeDescendent => {
                const elementDescendent = document.querySelector(`.${CSS.escape(classeDescendent)}`);
                if (elementDescendent) elementsAMostrar.add(elementDescendent);
            });
    
            // Afegir connexions internes del contenidor relacionat
            this.connexionsMap.forEach((infoConnexio, classeConnexio) => {
                if (!infoConnexio) return;
    
                const esStartIntern = infoConnexio.startNode.startsWith(nodeRelacionatDecodificat + '.');
                const esEndIntern = infoConnexio.endNode.startsWith(nodeRelacionatDecodificat + '.');
    
                if (esStartIntern && esEndIntern) {
                    const elementConnexio = document.querySelector(`.${CSS.escape(classeConnexio)}`);
                    if (elementConnexio) elementsAMostrar.add(elementConnexio);
                }
            });
        });
    
        // Aplicar canvis de visibilitat
